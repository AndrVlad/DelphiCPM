...
begin
  ...
  if Bytes[0] = 5 then
    Result := Result + '05'
  else if Bytes[0] = 7 then
    Result := Result + '07';
{  else
    логика на случай получения некорректной посылки }
...
begin
  ...
  PurgeComm(Phndl, PURGE_RXCLEAR or PURGE_TXCLEAR);
end;
}
