...
begin
  AngleThread:= TAngleThread.Create(False);
  AngleThread.Priority:=tpNormal;
  AngleThread.FreeOnTerminate:=true;
end;
...
procedure TwinMain.N1Click(Sender: TObject);
begin
      //вызов калькулятора
...
procedure TwinMain.RadioButton4Click(Sender: TObject);
begin
  AngleThread:= TAngleThread.Create(False);
  AngleThread.Priority:=tpNormal;
  AngleThread.FreeOnTerminate:=true;
...
procedure TwinMain.RadioButton5Click(Sender: TObject);
begin
   AngleThread.Terminate;
end;
